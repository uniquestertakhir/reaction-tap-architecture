...
function getPracticeRewardMeta(gameId: string) {
...
export default function ReactionTapRuntime({
  durationMs = 30_000,
  canPlay = true,
  gameId,
  mode,
  ...
  const urlGameId = sp.get("gameId");
  const urlMode = sp.get("mode");
  const urlModeId = sp.get("modeId");

  const effectiveGameId = (gameId || urlGameId || DEFAULT_GAME_ID).trim();
  const effectiveMode = (mode || urlMode || "warm-up").trim();
  const effectiveModeId = (urlModeId || "").trim();

  const gameConfig = getGameConfig(effectiveGameId);
  const rewardMeta = getPracticeRewardMeta(effectiveGameId);

  const resolvedModeMeta =
    gameConfig.modes.find((m) => m.id === effectiveModeId) ||
    gameConfig.modes.find((m) => m.mode === effectiveMode) ||
    null;

  const resolvedModeTitle = resolvedModeMeta?.title || effectiveMode;
  ...
  const [, setRuns] = useState<RunItem[] | null>(null);
  const [, setRunsError] = useState<string | null>(null);
  const [, setRunsLoading] = useState(false);

  const [practiceRewardAmount, setPracticeRewardAmount] = useState<number | null>(null);
  ...
  const onGameEndCalledRef = useRef<string | null>(null);
  ...
  function computeNextTargetRect(seed: number, nextId: number): Rect | null {
    ...
    const shrinkPerHit = Math.max(0.6, minSide * 0.0018);
    const rawSize = baseSize - nextId * shrinkPerHit;
    const size = clamp(
      rawSize,
      Math.max(36, Math.round(minSide * 0.08)),
      Math.round(minSide * 0.18)
    );
  ...
  function reset() {
    ...
    onGameEndCalledRef.current = null;
  }
  ...
  function start() {
    ...
    const now = Date.now();
    const d01 = ((Math.sin(now * 0.001) * 10000) % 1 + 1) % 1;
    const delayMs = 800 + Math.floor(d01 * 700);
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      try {
        ...
        const hasWinner = typeof matchInfo?.serverScore === "number";
  ...
  }, [matchId, matchInfo?.status, matchInfo?.serverScore]);
  ...
  useEffect(() => {
    ...
    if (!matchId) {
      const earned = rewardMeta.computePrize(score);

      try {
         commitRun({
          gameId: effectiveGameId,
          title: resolvedModeTitle,
          currency: rewardMeta.currency,
          prize: earned,
          matchId: null,
          mode: effectiveMode,
          score,
      ...
      setPracticeRewardAmount(earned);
      ...
      .then(async (data) => {
        ...
        if (data?.verified === true && typeof data.serverScore === "number") {
          ...
          try {
            ...
            const endJson = await r2.json().catch(() => null);

            const payoutAmount =
              typeof endJson?.payout?.amount === "number" ? endJson.payout.amount : 0;
  ...
  useEffect(() => {
    ...
    const endKey = `${matchId || "solo"}::${roundSeed || "no-seed"}::${score}`;
    if (onGameEndCalledRef.current === endKey) return;
    onGameEndCalledRef.current = endKey;

    onGameEnd({
      win: !!serverVerify?.verified,
      score,
      ms: durationMs - timeLeftMs,
    });
  }, [phase, onGameEnd, matchId, roundSeed, score, durationMs, timeLeftMs, serverVerify]);
  ...
  const isMatchMissing =
    !!matchId &&
    (matchPollError === "not_found" || matchPollError === "match_not_found");

  const effectiveCanPlay = !!canPlay && !isEnded && !isMatchMissing;

    const practiceRewardLabel =
    practiceRewardAmount === null
      ? null
      : `+${practiceRewardAmount} 💎 earned`;
  ...
  }, [matchId, matchInfo?.status, phase, effectiveCanPlay, playerId, effectiveGameId]);
          ...
          <div className="w-[86%] max-w-[360px] rounded-[28px] border border-white/10 bg-black/55 p-5 text-center shadow-[0_30px_110px_rgba(0,0,0,0.75)] backdrop-blur">
            ...
            {matchId ? (
              <div className="mt-3 text-[11px] text-white/50">
                Match mode starts automatically when ready.
              </div>
            ) : (
              <div className="mt-3 text-[11px] text-white/50">
                Practice rewards 💎 after the run.
              </div>
            )}
          </div>
        </div>
      ...
      {phase === "ended" && (
        <div className="absolute inset-0 z-50 grid place-items-center">
          <div className="w-[86%] max-w-[360px] rounded-[28px] border border-white/10 bg-black/60 p-6 text-center shadow-[0_30px_110px_rgba(0,0,0,0.75)] backdrop-blur">
            ...
            {serverVerify && !matchId ? (
              <div className="mt-2 text-xs text-white/60">Verified ✅</div>
            ) : null}
  ...
  return <div className="w-full">{Field}</div>;
}