// ===== FILE START: apps/web/components/game/BlockPuzzleRuntime.tsx =====
"use client";
...
function BlockPuzzleClientInner() {
...
const [audioSettings, setAudioSettings] = useState<AudioSettings>(() =>
  ...
  const [drag, setDrag] = useState<DragState>({ active: false });
    // ✅ prevent duplicate result commits
...
useEffect(() => {
  ...
  const gameConfig = getGameConfig("block-puzzle");
  ...
  const resolvedModeTitle = resolvedModeMeta?.title || mode;
    // ✅ commit result to platform Results (once) when game ends
  useEffect(() => {
    if (!mode) return;
    if (!gameOver) return;
    ...
    committedRunRef.current = true;
          ...
          const title = `Block Puzzle · ${resolvedModeTitle}`;

   commitRun({
      gameId: "block-puzzle",
      title,
      currency: resultCurrency,
      prize: Number.isFinite(prize) && prize > 0 ? prize : 0,
      mode,
      score,
      serverScore: score,
      verified: true,
      outcome: resultCurrency === "cash" ? "win" : "completed",
      durationMs: 0,
      placement: resultCurrency === "cash" ? 1 : null,
      rewardSource: resultCurrency === "cash" ? "cash" : "practice",
      walletDelta:
        resultCurrency === "gems" && Number.isFinite(prize) && prize > 0
          ? prize
          : 0,
    });

      }, [mode, modeId, gameOver, score, resultCurrency, prize, resolvedModeTitle]);
    // If opened without mode -> go to /modes (single entry point)
  React.useEffect(() => {
    if (!mode) router.replace("/games/block-puzzle/modes");
  }, [mode, router]);
...
export default function BlockPuzzleRuntime() {
  return (
    <Suspense fallback={null}>
      <BlockPuzzleClientInner />
    </Suspense>
  );
}
// ===== FILE END: apps/web/components/game/BlockPuzzleRuntime.tsx =====